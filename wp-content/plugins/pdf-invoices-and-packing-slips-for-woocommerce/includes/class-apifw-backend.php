...
class APIFW_Backend
{
    ...
    */
    public function __construct( $file = '', $version = '1.0.0' )
    {
        ...
        add_action( 'manage_awcdp_payment_posts_custom_column', array( $this, 'new_awcdp_list_column_content' ) );
    ...
    */
    ...
    {
        add_meta_box( 'apifw-order-metabox', __( 'Invoice & Labels', 'pdf-invoices-and-packing-slips-for-woocommerce' ), array( $this, 'get_order_metabox_content' ), 'shop_order', 'side', 'default' );
    }
    ...
    */
    ...
    {
        global $post;
        $order_invoice_no = get_post_meta( $post->ID, $this->_token.'_ord_invoice_no', true );
        ...
        if( $this->invoice_settings['status'] == true ):
            if( ! empty( $order_invoice_no ) ) {
                $not_formatted_flag = get_post_meta( $post->ID, $this->_token.'_invnum_not_formatted_flag', true );
                if( ! empty( $not_formatted_flag ) ) {
                    // generating class obj
                    $invoice_generator = new APIFW_Invoice();
                    $order_invoice_no = $invoice_generator->format_invoice_number( $order_invoice_no, $post->ID );
                }
                // content
    ...
    */
    public function handle_bulk_actions_ordertable()
    {
        ...
        if ( $action == 'apifw_invoice_preview' ) {
            $orderid_enc = urlencode( serialize( $order_ids_array ) );
    ...
    */
    public function get_deposit_metabox_content()
    {
        global $post;
		$order_invoice_no = get_post_meta( $post->ID, $this->_token.'_ord_invoice_no', true );
    ...
    */
    public function new_awcdp_list_column_content( $column )
	{
        global $post;
        $orderids_array = array( $post->ID );
        $orderid_enc = urlencode( serialize( $orderids_array ) );
    ...
    */
    public function admin_footer_scripts()
    {
        ...
        if( $currentScreen->id == 'edit-shop_order' || $currentScreen->id == 'edit-awcdp_payment' ) { ?>
            <script>
