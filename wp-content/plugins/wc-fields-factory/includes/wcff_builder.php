...
class wcff_builder {
				...
				<table class="wcff_table">
					<tbody>
						<tr>
                            ...
                            <td class="field-name">                                                               
                                <label class="wcff-field-name"><?php echo isset($field["key"]) ? $field["key"] : ""; ?></label>
                            </td>
							<td class="field-type">
    ...
    private function build_factory_meta_input_field($_meta, $_ptype="wccpf") {
    	...
    	return '<input type="'. $_meta["type"].'" name="wcff-field-type-meta-'. $_meta["param"] .'" class="wcff-field-type-meta-'. $_meta["param"] .'" placeholder="'. $_meta["placeholder"] .'" value="'. $value .'" />';
    }
    	...
    	<div class="wcff_fields_factory wcff_fields_factory_config_wrapper" action="POST" style="display: none;">
        	<div class="wcff_fields_factory_config_container">
            	...
            	<div class="wcff-factory-tab-header">
        			<a href=".wcff-factory-tab-fields-meta" class="selected"><?php _e( 'Field Meta', 'wc-fields-factory' ); ?></a>		
        			<?php if ($_field_type != "email" && $_field_type != "label" && $_field_type != "hidden" && $_field_type != "file") : ?>
        			<a href=".wcff-factory-tab-pricing-rules"><?php _e( 'Pricing Rules', 'wc-fields-factory' ); ?></a>	
        			<a href=".wcff-factory-tab-fields-rules"><?php _e( 'Field Rules', 'wc-fields-factory' ); ?></a>
        			<?php endif; ?>
        			<?php if ($_field_type == "colorpicker") : ?>
                	<a href=".wcff-factory-tab-color-image"><?php _e( 'Colors to Images', 'wc-fields-factory' ); ?></a>
                	<?php endif; ?>
        		</div>
        		<?php endif; ?>
    	...
    	<div class="wcff-factory-tab-child wcff-factory-tab-pricing-rules">			
    		<table class="wcff_table">
    			<tbody class="wcff-field-types-meta-body">
    				<tr>
    					<td class="summary">
    						<label for="post_type"><a href="https://sarkware.com/pricing-fee-rules-wc-fields-factory/" target="_blank" title="<?php _e( 'Documentation', 'wc-fields-factory' ); ?>"><?php _e( 'Click here for Documentation', 'wc-fields-factory' ); ?></a></label>
    						<br/>
    	...
    	<div class="wcff-factory-tab-child wcff-factory-tab-fields-rules">			
    		<table class="wcff_table">
    			<tbody class="wcff-field-types-meta-body">
    				<tr>
    					<td class="summary">
    						<label for="post_type"><a href="https://sarkware.com/field-rule-wc-fields-factory/" target="_blank" title="<?php _e( 'Documentation', 'wc-fields-factory' ); ?>"><?php _e( 'Click here for Documentation', 'wc-fields-factory' ); ?></a></label>
    						<br/>
    			...
    			<tbody class="wcff-field-types-meta-body">
    				<tr>
    					<td class="summary">
    						...
    						<p class="description"><?php _e( 'Choose your color pallet and perticular color based image.', 'wc-fields-factory' ); ?></p>
                            <br/>
    					...
    					<td style="vertical-align: top;" class="wcff-content-config-cell">
    						<div class="wcff-tab-rules-wrapper color-image">		
                               <div class="wcff-parent-rule-title"><?php _e( 'Color Image', 'wc-fields-factory' ); ?></div>	
                               <div class="wcff-rule-container">
    ...
    public function build_user_field($_meta, $_ptype = "wccpf", $_global_clonable = "no", $_group_clonable = "no", $_wrapper = true) { 
        ...
        if ($show_as_value == "no") {
            ...
            if ($_meta["type"] != "colorpicker") {
                if ($_meta["type"] != "checkbox") {
                    $html = '<p class="wcff-value-only-tag '. $field_class .'">'. $value .'</p>';
                } else {
                    if(!is_array($value)) {
                        $value = array();
                    }
                    $html = '<p class="wcff-value-only-tag '. $field_class .'">'. implode(", ", $value) .'</p>';
                }                
            } else {
    ...
    private function build_input_field($_meta, $_ptype = "wccpf", $_class = "", $_index = "", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        ...
        if (isset($_meta["for_front_end"])) {
            ...
            $attrs .= ' data-has_field_rules="'.$has_field_rules.'" data-has_pricing_rules="'.$_has_pricing_rules.'" '. $_cloneable .' class="wccpf-field ' . $_class .' ';
        ...
        if ($_meta["type"] != "colorpicker") {            
            ...
            $attrs .= ' value="'. (($value && $value != "") ? $value : "#000") .'" ';
        }
        ...
        if ($_meta["type"] == "text") {
            ...
            $html = '<input type="text" '. $attrs .' data-date-format="'. $dformat .'" data-pattern="mandatory" />';          
    ...
    private function build_radio_field($_meta, $_ptype = "wccpf", $_class = "", $_index = "", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        ...
        /* Common for all fields */
        $common_attrs = ' name="'. esc_attr($_meta["name"] . $_index) .'" data-fkey="'. $_meta["key"] .'" data-mandatory="'. $_meta["required"] .'" data-pattern="mandatory" '. $_readonly .' data-field-type="'. $_meta["type"] .'" ';
        if (isset($_meta["for_front_end"])) {
            /* Specific for front end */
            $common_attrs .= ' data-has_field_rules="'.$has_field_rules.'" data-has_pricing_rules="'.$_has_pricing_rules.'" '. $_cloneable .' class="wccpf-field ' . $_class . '" ';
        } else {
            $common_attrs .= ' class="'. $_ptype .'-field ' . $_class . '" ';
        }
    ...
    private function build_image_radio_choices($_meta, $_ptype = "wccpf", $_class = "", $_index = "", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        ...
        foreach ($choices as $choice) {
            ...
            if (count($key_val) == 2) {
                ...
                $html .= '<input type="radio" '. $common_attrs .' value="'. esc_attr(trim($key_val[0])) .'" '. $attr .' /><img src="'. $_meta["images"][trim($key_val[0])]["url"] .'" />';
    ...
    private function build_select_field($_meta, $_ptype = "wccpf", $_class = "", $_index = "", $_readonly = "", $_cloneable = "", $_wrapper = true, $_has_pricing_rules = "no") {
        ...
        if (isset($_meta["for_front_end"])) {
             /* Specific for front end */
            $html = '<select data-has_field_rules="'.$has_field_rules.'"  data-has_pricing_rules="'.$_has_pricing_rules.'" data-fkey="'. $_meta["key"] .'"  class="wccpf-field ' . $_class . '" name="' . esc_attr($_meta["name"] . $_index) . '" data-field-type="'. $_meta["type"] .'" data-pattern="mandatory" data-mandatory="' . $_meta["required"] . '" ' . $_cloneable . ' ' . $_readonly . ' >';
        } else {
             /* Specific for admin page */
            $html = '<select data-fkey="'. $_meta["key"] .'"  class="' . $_ptype . '-field ' . $_class . '" name="' . esc_attr($_meta["name"]) . '" data-field-type="'. $_meta["type"] .'" data-pattern="mandatory" data-mandatory="' . $_meta["required"] . '" ' . $_readonly . ' >';
        }       
    ...
    private function build_label_field($_meta, $_ptype, $_class = "", $_index = "", $_cloneable = "" ) {
        ...
        if ($_meta["message"] != "") {
            $html = '<div style="'.$onload_field.'" data-labelfield="'.$_meta["key"].'" data-fkey="'. $_meta["key"] .'" class="wcff-label wccpf_fields_table' . $_class . ' wcff-label-' . $_meta["message_type"] . '" '.$_cloneable.'><span class="wccpf-field label" data-fkey="'. $_meta["key"] .'">' . html_entity_decode($_meta["message"]) . '</span><input type="hidden" name="' . esc_attr($_meta["name"] . $_index) . '"></div>';
            if($_ptype == "wcccf"){
                $html = $this->built_field_wrapper($html, $_meta, $_ptype, $_index);
            }
            return $html;
        ...
        if ($_ptype != "wccaf") {
            if (isset($_meta["value"]) && $_meta["value"] != "") {
                ...
                if ($visual_type == "link") {
                    ...
                    $html = '<a href="' . $_meta["value"] . '" class="' . $_class . '" target="' . $open_tab . '" title="' . $_meta["tool_tip"] . '" ' . $_cloneable . ' >' . $_meta["link_name"] . '</a>';
                } else {
                    /* Admin wants this url to be displayed as Button */
                    $html = '<button onclick="window.open(\'' . $_meta["value"] . '\', \'' . $open_tab . '\' )"  title="' . $_meta["tool_tip"] . '" class="' . $_class . '" ' . $_cloneable . ' >' . $_meta["link_name"] . '</button>';
                }
            } else {
    ...
    private function built_field_wrapper($_html, $_meta, $_ptype, $_index) {
        ...
        if ($_ptype != "wccaf" && $_ptype != "wcccf" ) {
            ...
            if (has_filter('wccpf_custom_field_wrapper')) {
                ...
                $html .= '<td class="wccpf_value '. $label_alignment .'">'. ($label_alignment == "top" ? $label : ""). $_html . '</td>';
                $html .= '</tr>';
                $html .= '</tbody>';
                $html .= '</table>';
            }
