...
class wcff_injector {
	...
	public function inject_placeholder_for_variation_fields() {
	    ...
	    if ($product->is_type('variable')) {  
	        ...
	        if (wcff()->dao->has_variable_tab_fields) {
	            echo '<div id="wcff-variation-fields" class="wcff-variation-fields" data-area="'. current_action() .'"></div>';
	        }
	...
	public function enqueue_client_side_assets($isdate_css = false) { 
	    ...
	    var wccpf_opt = {
			...
			real_time_price_update : "<?php echo isset( $wccpf_options["enable_ajax_pricing_rules"] ) ? $wccpf_options["enable_ajax_pricing_rules"] : "disable"; ?>",
		    price_container_is : "<?php echo isset( $wccpf_options["ajax_pricing_rules_price_container"] ) ? $wccpf_options["ajax_pricing_rules_price_container"] : "default"; ?>",
	        price_container : "<?php echo isset( $wccpf_options["ajax_price_replace_container"] ) ? $wccpf_options["ajax_price_replace_container"] : ""; ?>",
	        price_details : "<?php echo isset( $wccpf_options["pricing_rules_details"] ) ? $wccpf_options["pricing_rules_details"] : "hide"; ?>",			
		...
		wp_enqueue_script('jquery-ui-i18n', wcff()->info['dir'].'assets/js/jquery-ui-i18n.min.js?v='. wcff()->info["version"]);
		// Jquery Time Picker script
		wp_enqueue_script('jquery-ui-timepicker-addon', wcff()->info['dir'].'assets/js/jquery-ui-timepicker-addon.min.js?v='. wcff()->info["version"]);
		/* Moment for date parsing */
		wp_enqueue_script('moment', wcff()->info['dir'].'assets/js/moment.min.js?v='. wcff()->info["version"]);
		// Color Picker css
		wp_enqueue_style("wcff-colorpicker-style", wcff()->info['dir'].'assets/css/spectrum.css?v='. wcff()->info["version"]);
		// Color Picker Script
		wp_enqueue_script('wcff-colorpicker-script', wcff()->info['dir'].'assets/js/spectrum.js?v='. wcff()->info["version"]);
		// wcff Client css 
		wp_enqueue_style("wcff-client-style", wcff()->info['dir'].'assets/css/wcff-client.css?v='. wcff()->info["version"]);
		//wcff Client Script
		wp_enqueue_script('wcff-client-script', wcff()->info['dir'].'assets/js/wcff-client.js?v='. wcff()->info["version"]);
        ...
        if ($_echo) { ?>
			...
			<?php 
			 	if (is_product() && $product) : ?> 
					var wcff_is_variable = "<?php echo ($product->is_type('variable')) ? "yes" : "no"; ?>";
				   var wcff_product_price = <?php echo $product->get_price(); ?>;
				<?php endif; ?>
